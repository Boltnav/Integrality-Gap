...
# # Loading the required C++ structs and functions
# import ctypes

# lib = ctypes.CDLL('./lib.so')

# # Calling the Rectangle struct
# # class rect_py(ctypes.Structure):
# #     _fields_ = [("bottomLeft", ctypes.)]

# # Calling functions:

# # true_or_false = lib.intervalsIntersect(tuple pair1, tuple pair2)

# # optimalCuts_return = lib.optimalCuts(...)
...
lib = ctypes.CDLL('./op_cuts_lib.so')

# Loading Struct Rectangle and helper functions
create_rectangle = lib.create_rectangle
create_rectangle.restype = ctypes.c_void_p
create_rectangle.argtypes = [ctypes.c_int, ctypes.c_int, ctypes.c_int, ctypes.c_int]

get_bottom_left = lib.get_bottom_left
get_bottom_left.argtypes = [ctypes.c_void_p, ctypes.POINTER(ctypes.c_int), ctypes.POINTER(ctypes.c_int)]

get_top_right = lib.get_top_right
get_top_right.argtypes = [ctypes.c_void_p, ctypes.POINTER(ctypes.c_int), ctypes.POINTER(ctypes.c_int)]

destroy_rectangle = lib.destroy_rectangle
destroy_rectangle.argtypes = [ctypes.c_void_p]

# Loading intervalsIntersect function
cplusplus_intervals_intersect = lib.intervalsIntersectWrapper
cplusplus_intervals_intersect.argtypes = [ctypes.c_int, ctypes.c_int, ctypes.c_int, ctypes.c_int]
cplusplus_intervals_intersect.restype = ctypes.c_bool

# Loading OptimalCuts function
class SimpleGeometryObject(ctypes.Structure):
    _fields_ = [("object", ctypes.c_void_p),
                ("type", ctypes.c_int)]

class SimpleReturnOptimalCuts(ctypes.Structure):
    _fields_ = [("flag", ctypes.c_bool),
                ("objects", ctypes.POINTER(SimpleGeometryObject)),
                ("count", ctypes.c_int)]
...
def stateFromRects(rects):
	...
	for i in range(N):
		for j in range(N):
			...
			if (cplusplus_intervals_intersect(( rects[i].bottomLeft[0] , rects[i].topRight[0] ) , ( rects[j].bottomLeft[0] , rects[j].topRight[0] ))):
			...
			if (cplusplus_intervals_intersect(( rects[i].bottomLeft[1] , rects[i].topRight[1] ) , ( rects[j].bottomLeft[1] , rects[j].topRight[1] ))):
	...
	for i in range(n_sessions):
		...
		if ((step-1) % 5 == 0):		# choose the rectangle
			...
			for rect in range(N):
				...
				if (cplusplus_intervals_intersect((x1, x2), (rectangles[rect].bottomLeft[0], rectangles[rect].topRight[0]))):
			...
			for rect in range(N):
				...
				if (cplusplus_intervals_intersect((y1, y2), (rectangles[rect].bottomLeft[1], rectangles[rect].topRight[1]))):
